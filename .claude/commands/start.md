...
## Configuração
- Peça ao usuário o input para esta sessão (você receberá um ou mais cards do TargetProcess para trabalhar) use o MCP do targetprocess para entender o contexto.
- Se não estivermos em uma feature branch com um nome que remeta ao conteúdo deste card, crie uma
- Certifique-se de que existe uma pasta .claude/sessions/<feature_slug> onde <feature_slug> é um identificador único para esta sessão (pode ser o slug da feature ou story)
...
Continue iterando junto com o humano até obter aprovação da arquitetura definida.

Uma vez que o architecture.md esteja finalizado e o humano concorde com o seu entendimento, informe ao humano que ELE deve prosseguir para o próximo passo através do comando /plan em uma nova sessão do Claude Code (use /clear ou abra uma nova sessão para limpar o contexto). Relembre-o que ele precisará executar o /warm-up na nova sessão antes de ir para o /plan.
